//#endregion Stoploss Configurator ─────────────────────────────────────── -->
(function () {
    /* ── state ─────────────────────────────────────────────────── */
    ...
    var requiredNodes = [
        configBtn, panel, closeBtn, cancelBtn, saveBtn, instruction, condSel,
        priceInput, spotDisp, ptsDiff, pctDiff, pnlBox, pnlVal, hoverLine,
        markerLine, shadeReg, markerUpper, markerLower, shadeUpper, shadeLower,
        savedBar, savedText, canvas
    ];
    ...
    function renderSavedMarkers() {
        ...
        if (slSavedUpper || slSavedLower) {
            ...
            var parts = [];
            if (slSavedUpper) parts.push('Above ≥ ₹' + slSavedUpper.price.toLocaleString('en-IN')
                + ' (' + fmtPnL(calcPnL(slSavedUpper.price)) + ')');
            if (slSavedLower) parts.push('Below ≤ ₹' + slSavedLower.price.toLocaleString('en-IN')
                + ' (' + fmtPnL(calcPnL(slSavedLower.price)) + ')');
            savedText.textContent = 'SL: ' + parts.join('  |  ');
        } else {
