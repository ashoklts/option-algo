...
from scanner.service import get_index_historical_chart_bars
...
class _AlertChecker:
    ...
    def get_active_indicator_resolutions(self) -> set[str]:
        """Distinct indicatorResolution values across every alert whose
        chain needs the bar-close engine — drives start_indicator_alert_
        scheduler_loop's sleep duration."""
        db = MongoData()._db
        alerts_col = db[ALERTS_COLLECTION]
    ...
    def check_indicator_alerts(self) -> None:
        ...
        versa) — every condition in the chain must match the same bar."""
        db = MongoData()._db
        alerts_col = db[ALERTS_COLLECTION]
        ...
        for alert in alerts:
            ...
            resolution = alert.get("indicatorResolution")
            if not (alert_id and resolution):
                continue
