...
SCHEMA_PATH = ROOT_DIR / "algoreq" / "algo-strategy-response.json"
...
def _field_from_schema(field_key: str, required: bool) -> IndicatorField:
...
def _build_confirmed_fields(schema_key: str, schema: dict[str, Any]) -> tuple[IndicatorField, ...]:
    ...
    return tuple(_field_from_schema(field_key, field_key in required) for field_key in field_keys)
...
def build_indicator_catalog() -> tuple[IndicatorDefinition, ...]:
    ...
    for label in _parse_dropdown_labels(DROPDOWN_HTML_PATH):
