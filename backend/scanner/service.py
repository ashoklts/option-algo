...
DEFAULT_SCANNER_INDEXES: list[dict[str, Any]] = [
    {"lookup_symbol": "GOLDBEES", "raw_symbol": "GOLDBEES", "normalized_symbol": "gold_bees", "fetch_mode": "nse"},
...
# Maps DEFAULT_SCANNER_INDEXES lookup_symbol → (exchange, Kite tradingsymbol for quote API)
_KITE_INDEX_QUOTE_MAP: dict[str, tuple[str, str]] = {
    "NIFTY":          ("NSE", "NIFTY 50"),
    ...
    "NIFTYMIDCAP100": ("NSE", "NIFTY MIDCAP 100"),
    "NIFTYMIDCAP50":  ("NSE", "NIFTY MIDCAP 50"),
    "NIFTY100":       ("NSE", "NIFTY 100"),
    "NIFTYNXT50":     ("NSE", "NIFTY NEXT 50"),
    "NIFTYSMLCAP100": ("NSE", "NIFTY SMLCAP 100"),
...
_kite_cash_instrument_cache: tuple[dict, dict] | None = None
_kite_cash_instrument_cache_expires: float = 0.0
_kite_cash_instrument_cache_lock = threading.Lock()
...
def _get_cached_kite_cash_instrument_maps(kite) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    global _kite_cash_instrument_cache, _kite_cash_instrument_cache_expires
    with _kite_cash_instrument_cache_lock:
        if _kite_cash_instrument_cache and time.time() < _kite_cash_instrument_cache_expires:
            print("⚡ Using cached Kite instrument maps", flush=True)
            return _kite_cash_instrument_cache
    maps = _load_kite_cash_instrument_maps(kite)
    with _kite_cash_instrument_cache_lock:
        _kite_cash_instrument_cache = maps
        _kite_cash_instrument_cache_expires = time.time() + 6 * 3600  # 6-hour TTL
    return maps
...
def _build_scanner_daily_stock_instruments(db) -> tuple[list[str], dict[str, dict[str, Any]]]:
    rows = list(
        db[STOCKS_COLLECTION].find(
            {},
            {"_id": 0, "exchange": 1, "symbol": 1, "kite_token": 1, "series": 1},
        )
    )
    ...
    for row in rows:
        ...
        meta_map[instrument] = {
            ...
            "kite_token": str(row.get("kite_token") or "").strip(),
        }

    print(f"📘 Stock loader: total={len(rows)} loaded={len(instruments)} skipped={skipped}", flush=True)
    return instruments, meta_map


def _build_scanner_daily_index_instruments() -> tuple[list[str], dict[str, dict[str, Any]]]:
    instruments: list[str] = []
    meta_map: dict[str, dict[str, Any]] = {}
    seen: set[str] = set()
    ...
    for default_index in DEFAULT_SCANNER_INDEXES:
        lookup_symbol = str(default_index["lookup_symbol"]).strip().upper()
        quote_entry = _KITE_INDEX_QUOTE_MAP.get(lookup_symbol)
        if not quote_entry:
            continue
        exchange, kite_tradingsymbol = quote_entry
        instrument = f"{exchange}:{kite_tradingsymbol}"
        if instrument in seen:
            continue
        seen.add(instrument)
        instruments.append(instrument)
    ...
    print(f"📗 Index loader: loaded={len(instruments)}", flush=True)
    return instruments, meta_map


def _fetch_one_quote_batch(kite, idx: int, batch: list[str]) -> tuple[int, dict[str, Any], list[dict[str, Any]]]:
    preview = batch[:5]
    print(f"📦 Quote batch {idx} | tokens={len(batch)} | sample={preview}", flush=True)
    try:
        batch_quotes = kite.quote(batch) or {}
    except Exception as exc:
        print(f"❌ Quote batch {idx} failed: {exc}", flush=True)
        return idx, {}, [{"instrument": b, "reason": f"batch_error:{exc}"} for b in batch]

    missing = [inst for inst in batch if inst not in batch_quotes]
    print(f"✅ Quote batch {idx} done | fetched={len(batch_quotes)} | missing={len(missing)}", flush=True)

    missing_info: list[dict[str, Any]] = []
    for instrument in missing:
        try:
            single = kite.quote([instrument]) or {}
        except Exception as exc:
            missing_info.append({"instrument": instrument, "reason": f"retry_error:{exc}"})
            continue
        if instrument in single:
            batch_quotes[instrument] = single[instrument]
        else:
            missing_info.append({"instrument": instrument, "reason": "not_returned_by_quote_api"})

    return idx, batch_quotes, missing_info
...
def _fetch_kite_quotes_in_batches(kite, instruments: list[str], *, batch_size: int = 500) -> tuple[dict[str, Any], int, list[dict[str, Any]]]:
    ...
    batches = list(enumerate(_chunkify_strings(instruments, batch_size), start=1))
    total_batches = len(batches)
    print(
        f"🔢 Quote fetch: total_instruments={len(instruments)} batch_size={batch_size} total_batches={total_batches}",
        flush=True,
    )
    quotes: dict[str, Any] = {}
    missing_info: list[dict[str, Any]] = []

    with concurrent.futures.ThreadPoolExecutor(max_workers=min(total_batches, 6)) as pool:
        futures = {pool.submit(_fetch_one_quote_batch, kite, idx, batch): idx for idx, batch in batches}
        for future in concurrent.futures.as_completed(futures):
            _, batch_quotes, batch_missing = future.result()
            quotes.update(batch_quotes)
            missing_info.extend(batch_missing)

    if missing_info:
        print(f"❌ Total missing after all batches: {len(missing_info)}", flush=True)
...
def _bulk_insert_scanner_daily_quotes(
    target_col,
    ...
    *,
    ...
    key_field, ts_field = key_fields
    timestamps = sorted({str(doc.get(ts_field) or "").strip() for doc in docs if doc.get(ts_field)})
    existing_primary_keys: set[tuple[str, str]] = set()
    existing_fallback_keys: set[tuple[str, str]] = set()

    if timestamps:
        projection = {"_id": 0, key_field: 1, ts_field: 1}
        if fallback_symbol_field:
            projection[fallback_symbol_field] = 1
        for row in target_col.find({ts_field: {"$in": timestamps}}, projection):
            row_ts = str(row.get(ts_field) or "").strip()
            row_key = str(row.get(key_field) or "").strip()
            if row_key and row_ts:
                existing_primary_keys.add((row_key, row_ts))
            if fallback_symbol_field:
                fallback_key = str(row.get(fallback_symbol_field) or "").strip()
                if fallback_key and row_ts:
                    existing_fallback_keys.add((fallback_key, row_ts))
    ...
    for doc in docs:
        key_value = str(doc.get(key_field) or "").strip()
        ts_value = str(doc.get(ts_field) or "").strip()
        if not ts_value:
        ...
        fallback_value = ""
        if fallback_symbol_field:
            fallback_value = str(doc.get(fallback_symbol_field) or "").strip()

        primary_key = (key_value, ts_value) if key_value else None
        fallback_key = (fallback_value, ts_value) if fallback_value else None

        if not primary_key and not fallback_key:
            skipped_missing_key += 1
            continue

        if primary_key and primary_key in existing_primary_keys:
            skipped_existing += 1
            continue
        if fallback_key and fallback_key in existing_fallback_keys:
            skipped_existing += 1
            continue
    ...
    if not pending_docs:
        print(
            f"💾 Inserted 0 new {label} records into {target_col.name} (insert-only). "
            f"skipped_existing={skipped_existing} skipped_missing_key={skipped_missing_key}",
            flush=True,
        )
        return 0

    inserted_total = 0
    for batch_no, start in enumerate(range(0, len(pending_docs), batch_size), start=1):
        batch = pending_docs[start:start + batch_size]
        try:
            result = target_col.insert_many(batch, ordered=False)
            inserted = len(result.inserted_ids)
        except BulkWriteError as exc:
            inserted = int((exc.details or {}).get("nInserted") or 0)
        inserted_total += inserted
        print(f"💾 Batch inserted {inserted} docs (batch {batch_no}).", flush=True)

    print(
        f"💾 Inserted {inserted_total} new {label} records into {target_col.name} (insert-only). "
        f"skipped_existing={skipped_existing} skipped_missing_key={skipped_missing_key}",
        flush=True,
    )
...
def sync_scanner_daily_market_snapshot() -> dict[str, Any]:
    ...
    index_instruments, index_meta_map = _build_scanner_daily_index_instruments()
    print(f"✅ Loaded {len(stock_instruments)} stock instruments, {len(index_instruments)} index instruments", flush=True)

    # Fetch stock and index quotes in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        stock_future = pool.submit(_fetch_kite_quotes_in_batches, kite, stock_instruments, batch_size=500)
        index_future = pool.submit(_fetch_kite_quotes_in_batches, kite, index_instruments, batch_size=500)
        stock_quotes, stock_batches, stock_missing = stock_future.result()
        index_quotes, index_batches, index_missing = index_future.result()
    ...
    print(f"✅ Transformed EQ: {len(stock_docs)}, INDEX: {len(index_docs)}", flush=True)

    stock_done = _bulk_insert_scanner_daily_quotes(
    ...
    index_done = _bulk_insert_scanner_daily_quotes(
    ...
    print(
        f"🎉 Snapshot completed. stock_inserts={stock_done} index_inserts={index_done} elapsed={elapsed}s",
        flush=True,
    )
    if stock_missing:
    ...
    return {
        ...
        "stock_inserts": stock_done,
        "index_inserts": index_done,
...
def sync_scanner_historical_data(
    ...
    try:
        ...
        processed_items = 0
        stop_reason = ""
        all_stock_docs: list[dict[str, Any]] = []
        all_index_docs: list[dict[str, Any]] = []
        ...
        for _, index_row in sorted(index_rows.items()):
            ...
            all_index_docs.extend(docs)
            indexes_processed += 1
            index_records_inserted += len(docs)
            progress_print(f"   ✅ Queued {len(docs)} records (total queued: {index_records_inserted})")
        ...
        if not stop_reason:
            for row in stock_rows:
                ...
                all_stock_docs.extend(docs)
                stocks_processed += 1
                stock_records_inserted += len(docs)
                progress_print(f"   ✅ Queued {len(docs)} records (total queued: {stock_records_inserted})")
        ...
        # Bulk insert all collected docs in one shot
        if all_index_docs:
            progress_print(f"\n💾 Inserting {len(all_index_docs)} index records...")
            index_collection.insert_many(all_index_docs, ordered=False)
        if all_stock_docs:
            progress_print(f"💾 Inserting {len(all_stock_docs)} stock records...")
            stock_collection.insert_many(all_stock_docs, ordered=False)
...
def _sync_one_universe(
    ...
    new_stock_docs: list[dict[str, Any]] = []

    all_symbols_sorted = sorted({s.strip().upper() for s in nse_symbols if s.strip()})
    existing_symbols_in_db: set[str] = set(
        db[STOCKS_COLLECTION].distinct("symbol", {"symbol": {"$in": all_symbols_sorted}})
    )
    ...
    for symbol in all_symbols_sorted:
        ...
        if symbol in existing_symbols_in_db:
            skipped_existing.append(symbol)
        else:
            new_stock_docs.append({
                "symbol": symbol,
    ...
    if new_stock_docs:
        db[STOCKS_COLLECTION].insert_many(new_stock_docs, ordered=False)
...
def backfill_stocks_list_kite_tokens() -> dict[str, Any]:
    ...
    new_universe_stock_docs: list[dict[str, Any]] = []
    ...
    for rec in universe_records:
        ...
        for sym in (rec.get("stocks") or []):
            ...
            new_universe_stock_docs.append({
                "symbol": sym,
                "tradingsymbol": sym,
    ...
    if new_universe_stock_docs:
        db[STOCKS_COLLECTION].insert_many(new_universe_stock_docs, ordered=False)
...
def backfill_missing_tokens() -> dict[str, Any]:
    ...
    # ── Stocks: pre-load existing rows ─────────────────────────────────────────
    existing_stock_rows = {
        str(r["symbol"]): r
        for r in db[STOCKS_COLLECTION].find(
            {"symbol": {"$in": _MISSING_STOCK_SYMBOLS}},
            {"_id": 1, "kite_token": 1, "company_name": 1},
        )
    }

    for symbol in _MISSING_STOCK_SYMBOLS:
        row = existing_stock_rows.get(symbol)
        ...
        if resolved_token:
            ...
            if row:
                stock_ops.append(UpdateOne({"_id": row["_id"]}, {"$set": update_fields}))
            else:
                stock_ops.append(UpdateOne({"symbol": symbol}, {"$set": {"symbol": symbol, **update_fields}}, upsert=True))
            stocks_updated.append({"symbol": symbol, "token": resolved_token, "exchange": exchange, "company_name": company_name})
        else:
            db_company = str((row or {}).get("company_name") or "").strip()
            kite_company = str((inst or {}).get("name") or "").strip() if inst else ""
            stocks_not_found.append({"symbol": symbol, "company_name": db_company or kite_company or symbol})

    if stock_ops:
        db[STOCKS_COLLECTION].bulk_write(stock_ops, ordered=False)
    ...
    stock_index_ops: list[UpdateOne] = []

    existing_idx_rows = {
        str(r.get("symbol") or r.get("filter_symbol") or ""): r
        for r in db[INDEX_STOCKS_COLLECTION].find(
            {},
            {"_id": 1, "kite_token": 1, "symbol": 1, "filter_symbol": 1, "normalized_symbol": 1},
        )
    }
    existing_stock_index_rows = {
        str(r["symbol"]): r
        for r in db[STOCKS_COLLECTION].find(
            {"symbol": {"$in": _MISSING_INDEX_SYMBOLS}},
            {"_id": 1},
        )
    }
    ...
    for symbol in _MISSING_INDEX_SYMBOLS:
        ...
        idx_row = existing_idx_rows.get(kite_key) or existing_idx_rows.get(symbol.lower())
        ...
        if idx_token_ok:
            ...
            index_ops.append(UpdateOne(
                {"_id": idx_row["_id"]},
                {"$set": {"kite_token": token_str, "instrument_token": token_str,
                           "exchange_token": exchange_token_str, "normalized_symbol": normalized_symbol, "updated_at": now}},
            ))

        stock_row = existing_stock_index_rows.get(symbol)
        if stock_row:
            stock_index_ops.append(UpdateOne(
                {"_id": stock_row["_id"]},
                {"$set": {"kite_token": token_str, "instrument_token": token_str, "updated_at": now}},
            ))

        indexes_updated.append({"symbol": symbol, "kite_key": kite_key, "normalized_symbol": normalized_symbol, "token": token_str})

    if index_ops:
        db[INDEX_STOCKS_COLLECTION].bulk_write(index_ops, ordered=False)
    if stock_index_ops:
        db[STOCKS_COLLECTION].bulk_write(stock_index_ops, ordered=False)
