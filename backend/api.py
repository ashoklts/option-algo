...
def strategy_worker(args: dict):
    ...
    progress_dict = (args or {}).get("progress_dict")

    def on_progress(completed: int, total: int, day: str):
        if progress_dict is None:
            return
        try:
            progress_dict[strategy_id_str] = {
                "completed": completed,
                "total": total,
                "day": day,
            }
        ...
        if worker_args:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as pool:
                ...
                while futures:
                    ...
                    # Update progress based on running workers
                    total_pct = done_count * 100.0
                    active_day = f"Completed {done_count}/{total_strategies}"
                    for sid, info in progress_dict.items():
                        if info.get("total"):
                            worker_pct = (info["completed"] / info["total"]) * 100.0
                            # Keep portfolio progress from appearing stuck at 0%
                            # while each strategy is still in its initialization phase.
                            worker_pct = max(worker_pct, 5.0)
                            total_pct += worker_pct
                            active_day = info.get("day", active_day)
                    ...
                    for future in done:
                        ...
                        done_count += 1
                        if sid in progress_dict:
                            del progress_dict[sid]
                        del futures[future]
